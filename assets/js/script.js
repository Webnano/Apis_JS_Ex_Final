...
async function renderizar(montoPesos) {
    try {
        ...
        if (!isNaN(valorMoneda)) {
            ...
            document.getElementById('respuestaBoton').innerText = `${conversion.toFixed(2)} ${nombreMoneda}`;
        } else {
...
document.getElementById('botonBuscar').addEventListener('click', async () => {
    ...
    if (!isNaN(montoClp)) {
        ...
        document.getElementById('respuestaBoton').innerText = 'Ingrese un Valor a Convertir';
    }
});